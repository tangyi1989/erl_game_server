...
-export([encode_loginresponse/1, decode_loginresponse/1,
	 delimited_decode_loginresponse/1, encode_login/1,
...
-record(loginresponse, {result, description}).
...
encode_loginresponse(Records) when is_list(Records) ->
    delimited_encode(Records);
encode_loginresponse(Record)
    when is_record(Record, loginresponse) ->
    encode(loginresponse, Record).
...
encode(loginresponse, Records) when is_list(Records) ->
    delimited_encode(Records);
encode(loginresponse, Record) ->
    [iolist(loginresponse, Record)
...
iolist(loginresponse, Record) ->
    [pack(1, required,
	  with_default(Record#loginresponse.result, none), int32,
	  []),
     pack(2, required,
	  with_default(Record#loginresponse.description, none),
	  string, [])].
...
decode_loginresponse(Bytes) when is_binary(Bytes) ->
    decode(loginresponse, Bytes).
...
delimited_decode_loginresponse(Bytes) ->
    delimited_decode(loginresponse, Bytes).
...
decode(loginresponse, Bytes) when is_binary(Bytes) ->
    Types = [{2, description, string, []},
	     {1, result, int32, []}],
    Defaults = [],
    Decoded = decode(Bytes, Types, Defaults),
    to_record(loginresponse, Decoded).
...
to_record(login, DecodedTuples) ->
    Record1 = lists:foldr(fun ({_FNum, Name, Val},
			  ...
			  #login{}, DecodedTuples),
    Record1;
to_record(loginresponse, DecodedTuples) ->
    Record1 = lists:foldr(fun ({_FNum, Name, Val},
			       Record) ->
